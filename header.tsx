...
export const getServerSideProps: GetServerSideProps = async () => {
  ...
  try {
    // Fetch data from external API using JWTAccessToken for authorization
    ...
    if (res.ok) {
      ...
      const data: HeaderProps = await res.json();
      // Pass data to the page via props
      return { props: { repo: data } };
    } else {
      // Handle non-OK response
      console.error("Failed to fetch data:", res.statusText);
      return { props: { repo: null } }; // Or handle error accordingly
    }
  } catch (error) {
    // Handle fetch error
...
export default function Header({
  ...
  return (
    <main className="sticky left-0 right-0 top-0 z-20 border-b border-gray-200 bg-gray-300">
